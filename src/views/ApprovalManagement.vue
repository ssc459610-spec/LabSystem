            ...
            <div class="expand-content">
              <el-descriptions :column="2" border>
                ...
                <el-descriptions-item label="申请人角色">
                  <el-tag
                    :type="row.applicantRole === 'teacher' ? 'success' : 'info'"
                  >
                ...
                <el-descriptions-item label="借用数量"
                  >{{ row.quantity }} 台/套</el-descriptions-item
                >
                ...
                <el-descriptions-item label="借用时间" :span="2">
                  {{ row.borrowTime }} 至 {{ row.returnTime }} （共
                  {{ row.borrowDays }} 天）
                </el-descriptions-item>
                <el-descriptions-item
                  v-if="row.projectCode"
                  label="科研项目编号"
                  >{{ row.projectCode }}</el-descriptions-item
                >
                <el-descriptions-item
                  v-if="row.competitionProof"
                  label="竞赛凭证"
                >
                  <el-link
                    type="primary"
                    :href="row.competitionProof"
                    target="_blank"
                    >查看凭证</el-link
                  >
                </el-descriptions-item>
                ...
                <!-- 已审批的意见 -->
                <el-descriptions-item
                  v-if="row.approvalOpinions && row.approvalOpinions.length > 0"
                  label="审批意见"
                  :span="2"
                >
                  <div
                    v-for="opinion in row.approvalOpinions"
                    :key="opinion.step"
                    class="approval-opinion"
                  >
                    <el-tag
                      :type="
                        opinion.result === 'approved' ? 'success' : 'danger'
                      "
                    >
                      {{ opinion.approver }} -
                      {{ opinion.result === "approved" ? "通过" : "拒绝" }}
                    </el-tag>
    ...
    >
      <el-form :model="approvalForm" label-width="100px">
        <el-form-item label="审批结果">
          <el-tag
            :type="approvalForm.result === 'approved' ? 'success' : 'danger'"
          >
...
const filteredApprovals = computed(() => {
  ...
  if (filterType.value === "pending") {
    ...
    result = result.filter(
      (item) => !item.needMyApproval && item.status !== "pending",
    );
  }

  if (filterDeviceType.value) {
    result = result.filter(
      (item) => item.deviceType === filterDeviceType.value,
    );
  }
...
const confirmApproval = () => {
  ...
  // 更新本地数据（实际应该重新加载）
  const index = approvals.value.findIndex(
    (item) => item.applicationNo === approvalForm.applicationNo,
  );
  if (index !== -1) {
    if (approvalForm.result === "approved") {
      ...
      if (nextStep >= approvals.value[index].approvalFlow.length) {
        ...
        approvals.value[index].needMyApproval = false;
        approvals.value[index].currentApprover =
          approvals.value[index].approvalFlow[nextStep].description;
      }
    } else {
      // 拒绝
